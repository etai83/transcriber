...
class TranscriberService:
        ...
        try:
            ...
            if mean_volume is not None:
                return {
                    ...
                    "max_volume": max_volume or mean_volume,
                }
            return None
    ...
    @classmethod
    def normalize_audio(
        cls, audio_path: str, target_db: float = -20.0
    ) -> Optional[str]:
        """
        ...
        print(
            f"Audio volume analysis: mean={mean_volume:.1f} dB, max={stats['max_volume']:.1f} dB"
        )
        ...
        if abs(gain) < 10:
            print(
                f"Audio volume is acceptable (gain would be {gain:.1f} dB), skipping normalization"
            )
            return None
        ...
        if gain > max_gain:
            print(
                f"Limiting gain from {gain:.1f} dB to {max_gain} dB to prevent issues"
            )
            gain = max_gain
        ...
        try:
            ...
            cmd.extend(
                [
                    ...
                    audio_path,
                    "-af",
                    f"volume={gain}dB,alimiter=limit=0.95:attack=5:release=50",
                    "-ar",
    ...
    @classmethod
    def trim_silence(
        cls, audio_path: str, threshold_db: str = "-30dB", min_duration: float = 0.5
    ) -> str:
        """
        ...
        try:
            ...
            cmd.extend(
                [
                    ...
                    audio_path,
                    "-af",
                    f"silenceremove=start_periods=1:start_duration={min_duration}:start_threshold={threshold_db}:stop_periods=-1:stop_duration={min_duration}:stop_threshold={threshold_db}",
                    "-c:a",
                    "libmp3lame",
                    "-q:a",
                    "2",
    ...
    @classmethod
    def transcribe(
        cls, audio_path: str, language: str = "auto", trim_silence: bool = False
    ) -> Dict[str, Any]:
        """
        ...
        if len(words) >= 4:
            ...
            for word, count in word_counts.items():
                if count >= 4 and count / len(words) > 0.4:
                    print(
                        f"Detected repetitive word hallucination: '{word}' appears {count} times in {len(words)} words"
                    )
                    return True
        ...
        if len(words) >= 6:
            ...
            for pattern_len in range(1, min(5, len(words) // 3 + 1)):
                ...
                if count >= 3 and len(pattern) * count > len(text_lower) * 0.5:
                    print(
                        f"Detected repetitive phrase hallucination: '{pattern}' repeated {count} times"
                    )
                    return True
    ...
    def get_supported_languages(cls) -> Dict[str, str]:
        ...
        return {"auto": "Auto-detect", "en": "English", "he": "Hebrew"}
    ...
    def transcribe_with_diarization(
        ...
        try:
            ...
            result = {
                ...
                "duration": whisper_result.get("segments", [{}])[-1].get("end", 0)
            ...
            print(
                f"Running speaker diarization with {num_speakers} speakers on: {transcribe_path}"
            )
            ...
            if transcript_segments.get("full_text"):
                print(
                    f"Diarization complete: {len(transcript_segments['speakers'])} speakers detected"
                )
