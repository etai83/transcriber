...
class DiarizerService:
    ...
    def get_pipeline(cls):
        ...
        if cls._pipeline is None:
            ...
            embedding_model = (
                MODELS_DIR / "wespeaker-voxceleb-resnet34-LM" / "pytorch_model.bin"
            )
            ...
            if segmentation_model.exists() and embedding_model.exists():
                try:
                    ...
                    print(
                        f"Diarization pipeline loaded from local models on {cls._device}"
                    )
    ...
    def merge_with_transcript(
        cls, whisper_segments: List[Dict], diarization_segments: List[Dict]
    ) -> Dict[str, Any]:
        """
    ...
    def _find_speaker_for_segment(
        cls, start: float, end: float, diarization_segments: List[Dict]
    ) -> str:
        """
